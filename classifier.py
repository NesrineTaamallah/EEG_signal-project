...
balanced_accuracies = []
roc_aucs = []
last_cm = None   # capture the last fold's confusion matrix

print("\nDébut de la validation croisée...\n")

splitter = cv.split(X, y, groups) if use_stratified else cv.split(X, y, groups)
...
# ── Save metrics so the FastAPI /metrics endpoint returns real values ────────
metrics_json_path = os.path.join(os.path.dirname(MODEL_PATH), "metrics.json")
